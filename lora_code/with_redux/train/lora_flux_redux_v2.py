#!/usr/bin/env python3
"""
Train LoRA on FLUX with Redux image conditioning - V2.
Improvements:
- Use u directly as timestep (no noise_scheduler mapping)
- Scale Redux embeddings with REDUX_SCALE during training
- Log gradient norms for monitoring convergence
"""
...
def parse_args():
    import argparse
    parser = argparse.ArgumentParser(description="Train Flux LoRA with Redux - V2")
...
def main():
    ...
    # Combine text and redux embeddings (with scaling)
    print(f"Combining text and redux embeddings (Redux scale: {args.redux_scale})...")
    combined_prompt_embeds = []
    combined_pooled_embeds = []

    for i in range(len(dataset)):
        # Scale Redux embeddings (match inference)
        redux_prompt_scaled = all_redux_prompt_embeds[i] * args.redux_scale
        redux_pooled_scaled = all_redux_pooled_embeds[i].squeeze(0) * args.redux_scale

        # Combine text embeddings and scaled Redux image embeddings
        prompt_embeds = torch.cat([text_embeds_t5.unsqueeze(0), redux_prompt_scaled], dim=1)
        pooled_embeds = text_pooled_embeds + redux_pooled_scaled
    ...
    # Training loop
    print("="*60)
    print(f"Starting training - LoRA Rank {args.rank}")
    ...
    for epoch in range(999):  # Large number, will break when max_steps reached
        for batch in train_dataloader:
            with accelerator.accumulate(transformer):
                ...
                timesteps = u  # still use u directly

                # Define path from noise -> data:
                # x(u) = (1 - u) * noise + u * latents
                ...
                # Keep as 2D [L, 3] (batch dimension handled internally by transformer)
                ...
                # Flow-matching target: data - noise (velocity pointing from noise to data)
                target = (latents_packed - noise_packed).to(torch.float32)
                loss = F.mse_loss(model_pred, target, reduction="mean")

                accelerator.backward(loss)

                # V2: Calculate gradient norm before clipping
                grad_norm = None
    ...
    if accelerator.is_main_process and len(loss_history) > 0:
        ...
        # Save plot
        plot_path = os.path.join(args.output_dir, f"training_metrics_rank{args.rank}.png")
